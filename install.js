// install.js — robust install with fallback + memory
...
window.addEventListener('DOMContentLoaded', ()=>{
  if (maybeStandalone()){
    hideInstallButton(); showInstalledBadge();
    localStorage.setItem('via_install_seen','1');
    const btn = document.getElementById('install');
...
window.addEventListener('appinstalled', ()=>{
  showInstalledBadge(); hideInstallButton();
  localStorage.setItem('via_install_seen','1');
});
